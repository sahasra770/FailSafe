...
# G3 removed — it's the target variable, not a feature
FEATURE_NAMES = [
...
def get_intervention(row, risk_label):
    ...
    if row.get('G2', 20) < 10:                      # use G2 instead of G3
    ...
    if primary == 'absences':
        ...
        g2 = int(reasons[0][1])
        return "Academic Tutoring", f"{prefix}Student scored {g2}/20 in mid-term 2. Provide personalized study plan and extra tutorial sessions for weak subjects"
...
def seed():
    ...
    for i, row in df.iterrows():
        ...
        # Use G2 as best available grade proxy (G3 is target, excluded from features)
        cgpa = round((row['G2'] / 20.0) * 10, 2)
